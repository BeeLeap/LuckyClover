...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'news_save') {
        ...
        if ($id && $name && $type && $host && $port > 0) { $stmt = $db->prepare('UPDATE servers SET name=?, type=?, host=?, port=?, enabled=?, updated_at=? WHERE id=?'); $stmt->execute([$name, $type, $host, $port, $enabled, $now, $id]); $message = '服务器配置已保存。'; }
